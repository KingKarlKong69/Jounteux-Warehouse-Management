...
const exportUrl = (type) => {
    ...
    if (type === 'csv') return route('admin.products.export.csv') + '?' + params.toString();
    if (type === 'excel') return route('admin.products.export.excel') + '?' + params.toString();
    if (type === 'report') return route('admin.products.report') + '?' + params.toString();
    return '#';
};
...
const restoreProduct = (id) => {
    confirm({
        ...
        onConfirm: () => {
            router.put(route('admin.products.restore', id), {}, {
                preserveScroll: true,
    ...
    <AuthenticatedLayout>
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="mt-4 sm:mt-0 flex items-center gap-2">
                <!-- Add Product Button (only show when not in archives view) -->
                <Link
                    v-if="!showDeleted"
                    ...
                    <tr v-for="product in products.data" :key="product.id" class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <Link :href="route('admin.products.show', product.id)" class="text-sm font-medium text-gray-900 hover:text-customOrange">{{ product.name }}</Link>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div v-if="!showDeleted" class="flex items-center justify-end gap-1">
                                <Link :href="route('admin.products.show', product.id)" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded" title="View">
                                    <EyeIcon class="h-4 w-4" />
                                </Link>
                                <Link :href="route('admin.products.edit', product.id)" class="p-1.5 text-gray-600 hover:bg-gray-100 rounded" title="Edit">
                                    <PencilIcon class="h-4 w-4" />
                                </Link>
        ...
        <!-- Pagination -->
        <Pagination :meta="products.meta" label="products" @navigate="goToPage" />
    </AuthenticatedLayout>
</template>