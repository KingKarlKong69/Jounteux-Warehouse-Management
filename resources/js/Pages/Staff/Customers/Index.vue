...
const submitForm = () => {
    if (modalMode.value === 'add') {
        form.post(route('staff.customers.store'), {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
        ...
        form.put(route('staff.customers.update', editingCustomerId.value), {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
...
const confirmDelete = () => {
    ...
    router.delete(route('staff.customers.destroy', customerToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
...
const restoreCustomer = (id) => {
    confirm({
        ...
        onConfirm: () => {
            router.put(route('staff.customers.restore', id), {}, {
                preserveScroll: true,
                onSuccess: () => {
...
const forceDeleteCustomer = (id) => {
    confirm({
        ...
        destructive: true,
        onConfirm: () => {
            router.delete(route('staff.customers.force-delete', id), {
                preserveScroll: true,
                onSuccess: () => {
...
// Pagination
const currentPage = computed(() => props.customers?.meta?.current_page || 1);
const lastPage = computed(() => props.customers?.meta?.last_page || 1);
const maxVisible = 5;
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <!-- Pagination -->
            <div v-if="customers?.meta?.last_page > 1" class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Showing <span class="font-medium">{{ customers.meta.from }}</span> to
                            <span class="font-medium">{{ customers.meta.to }}</span> of
                            <span class="font-medium">{{ customers.meta.total }}</span> results
                        </p>
                    </div>
                    <div>
