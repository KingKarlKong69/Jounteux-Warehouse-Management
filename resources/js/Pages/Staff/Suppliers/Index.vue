...
const applyFilters = () => {
    router.get(route('staff.suppliers.index'), {
...
const submitForm = () => {
    if (modalMode.value === 'add') {
        form.post(route('staff.suppliers.store'), {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
        ...
        form.put(route('staff.suppliers.update', editingSupplierId.value), {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
...
const confirmDelete = () => {
    ...
    router.delete(route('staff.suppliers.destroy', supplierToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
...
const restoreSupplier = (id) => {
    confirm({
        ...
        onConfirm: () => {
            router.put(route('staff.suppliers.restore', id), {}, {
                preserveScroll: true,
                onSuccess: () => {
...
const forceDeleteSupplier = (id) => {
    confirm({
        ...
        destructive: true,
        onConfirm: () => {
            router.delete(route('staff.suppliers.force-delete', id), {
                preserveScroll: true,
                onSuccess: () => {
...
// Pagination
const currentPage = computed(() => props.suppliers?.meta?.current_page || 1);
const lastPage = computed(() => props.suppliers?.meta?.last_page || 1);
const maxVisible = 5;
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <!-- Pagination -->
            <div v-if="suppliers?.meta?.last_page > 1" class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Showing <span class="font-medium">{{ suppliers.meta.from }}</span> to
                            <span class="font-medium">{{ suppliers.meta.to }}</span> of
